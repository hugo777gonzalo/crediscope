...
// El panorama de la cartera, contado en la base (migración 081).
//
// Antes se bajaban los perfiles con `limit(5000)` y se contaba en el
// navegador. PostgREST corta en 1.000 filas sin avisar, así que el
// panorama contaba ~840 clientes de 2.807 (medido el 2026-09-24): decía
// 290 dependientes privados donde había 950. Ahora llega el resultado, y
// alcanza para toda la cartera.
export async function getResumenCarteraIngresos() {
  const { data, error } = await supabase.rpc("resumen_fuentes_ingreso");
  if (error) throw error;
  return data;
}
